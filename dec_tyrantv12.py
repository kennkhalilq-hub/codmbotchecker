...
class DataDomeManager:
    ...
    def handle_403(self, session, telegram_config=None):
        ...
        time.sleep(0.2)  # brief settle before fetching datadome
...
class LiveStats:
    ...
    def get_cpm(self):
        """Return checks per minute based on elapsed time since start."""
        with self.lock:
            elapsed = time.time() - self.start_time
            if elapsed < 1:
                return 0
            return round(self.total_processed / elapsed * 60, 1)
    ...
    def get_stats(self):
        with self.lock:
            elapsed = time.time() - self.start_time
            cpm = round(self.total_processed / elapsed * 60, 1) if elapsed >= 1 else 0
            return {
...
def get_codm_access_token(session):
    ...
    try:
        ...
        grant_response = session.post(grant_url, headers=grant_headers, data=grant_data, timeout=8)
...
def process_codm_callback(session, access_token, open_id=None, uid=None):
    ...
    try:
        ...
        old_response = session.get(old_callback_url, headers=old_headers, allow_redirects=False, timeout=8)
        ...
        aos_response = session.get(aos_callback_url, headers=aos_headers, allow_redirects=False, timeout=8)
...
def select_input_file():
    ...
    print(f"{CYAN}║{RESET}  {WHITE}Lines: {MAGENTA}{selected_file['lines']:,}{' ' * (40 - len(str(format(selected_file['lines'], ','))))}{RESET} {CYAN}║{RESET}")
    print(f"{CYAN}╚{'═' * 50}╝{RESET}")
    print()
