...
def run_checker(uid,combo_file,result_folder,limit,threads,stop_event,
    ...
    _SESSION_RECYCLE = 100  # recycle session every 100 accounts — fewer recreations = faster
    ...
    try:
        ...
        try:
            ...
            _BATCH = MAX_WORKER_THREADS * 4  # 4x threads keeps pipeline full without OOM
...
async def on_callback(update,context):
    ...
    if data.startswith("adm_"):
        ...
        if data.startswith("adm_genkey_"):
            ...
            key3=f"SALTCODM-{_uuid.uuid4().hex[:8].upper()}-{_uuid.uuid4().hex[:4].upper()}"
        ...
        if data in ("adm_genkey_hours","adm_genkey_days","adm_genkey_months","adm_genkey_lifetime"):
            ...
            key=f"SALTCODM-{_uuid2.uuid4().hex[:8].upper()}-{_uuid2.uuid4().hex[:4].upper()}"
    ...
    if data=="start_check":
        ...
        def _status_loop():
            _tick = [0]
            while not _status_stop.wait(10):
                _tick[0] += 1
                with sessions_lock: s2 = active_sessions.get(uid, {})
                if s2.get("status") != "checking": break

                # ── Update stats card (in-memory only — no file I/O) ──
                ls2 = s2.get("live_stats")
                if ls2 is not None:
                    cur_stats = ls2.get_stats()
                    cur_stats['cpm'] = ls2.get_cpm() if hasattr(ls2, 'get_cpm') else 0
                    # ── Persist stats snapshot every 60 s (not every tick — disk I/O is slow) ──
                    if _tick[0] % 6 == 0:
                    ...
                    # Heavy file scan only every 6 ticks (≈60 s) to avoid I/O slowdown
                    _rf_arg = str(rf) if _tick[0] % 6 == 0 else None
                    card = stats_card(done_count, disp, cur_stats, ll, cl,
                                       result_folder=_rf_arg)
                    try:
                ...
                # Only check folder size every 6 ticks (≈60 s) — rglob is expensive
                if _tick[0] % 6 == 0:
                    try:
...
async def cmd_generate_key(update,context):
    ...
    key=f"SALTCODM-{uuid.uuid4().hex[:8].upper()}-{uuid.uuid4().hex[:4].upper()}"
                    ...
                    def _loop():
                        ...
                        while not sstop.wait(10):
                            ...
                            if ls3 is not None:
                                ...
                                cur3_raw['cpm'] = ls3.get_cpm() if hasattr(ls3,'get_cpm') else 0
                                if _tick3[0] % 6 == 0:
                                ...
                                if prev_s:
                                    ...
                                    display3=dict(cur3_raw)
                                    display3["total"]=done3
                                # Heavy file scan only every 6 ticks (≈60 s)
                                _rf3_arg=str(rf_base) if _tick3[0] % 6 == 0 else None
                                card3=stats_card(done3,orig_n,display3,ll_s,cl_s,
                                                 result_folder=_rf3_arg)
                                if msg_id:
                                    try:
                                        asyncio.run_coroutine_threadsafe(
                            ...
                            # ── Auto-send when near 49 MB (every 6 ticks ≈ 60 s) ──
                            if _tick3[0] % 6 == 0:
                                try:
